function init() {
  ...
  const triggerIndicatorChange = () => {
    pageBoundaries().forEach((ele, i) => {
      if (!windowDivs[i + 1]) {
        window.scrollY >= ele ? activate(i) : deactivate(i);
        return;
      }
  ...
  const scrambleLetter = (target, scrambledLetters, shuffle, text, speed) => {
    ...
    if (target.textContent.length < text.length) {
      if (shuffle < shuffleCount) {
        ...
        target.innerText =
          output + random[Math.floor(Math.random() * random.length)];
        interval = setTimeout(() => {
  ...
  const scrollAnimateLoad = function () {
    scrollAnimateElm.forEach((_ele, i) => {
      ...
      window.innerHeight >
      scrollAnimateElm[i].getBoundingClientRect().top + switchMargin
    ...
    window.pageYOffset === 0 ? triggerTopState() : stopTopState();
  };
