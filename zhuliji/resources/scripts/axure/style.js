$axure.internal(function($ax) {
    ...
    var _hoverOverride = function(id) {
        ...
        if($ax.style.IsWidgetError(id)) return ERROR;
    ...
    $ax.style.SetWidgetMouseDown = function(id, value, checkMouseOver) {
        ...
        if($ax.style.IsWidgetError(id)) return;
    ...
    $ax.style.IsWidgetError = function (id) {
        return Boolean( _errorWidgets[id] || $('#' + id).hasClass(ERROR));
    };
    ...
    $ax.style.SetWidgetError = function (id, value) {
        ...
        if(!_widgetHasState(id, ERROR) && !($('#' + id).hasClass(ERROR))) return;
        var state = null;
        if(!value) state = _generateState(id);
        else state = ERROR;
        _applyImageAndTextJson(id, state);
        _updateElementIdImageStyle(id, state);
    }
    ...
    var ALL_STATES = [MOUSE_OVER, MOUSE_DOWN, SELECTED, FOCUSED, SELECTED_FOCUSED, DISABLED, ERROR];

    var _generateState = _style.generateState = function (id) {
        return $ax.placeholderManager.isActive(id) ? HINT : _style.IsWidgetDisabled(id) ? DISABLED : _style.IsWidgetError(id) ? ERROR :
            _generateSelectedState(id, _style.IsWidgetSelected(id)); 
    };
