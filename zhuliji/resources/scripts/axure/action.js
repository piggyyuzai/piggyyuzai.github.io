...
//                $ax.event.raiseSyntheticEvent(elementId, "onMove");
    ...
    _actionHandlers.enableDisableWidgets = function (eventInfo, actions, index) {
        ...
        for(let i = 0; i < action.pathToInfo.length; i++) {
            let elementIds = $ax.getElementIdsFromPath(action.pathToInfo[i].objectPath, eventInfo);
            let enableAction = action.pathToInfo[i].enableDisableInfo.enableAction;
            let isToggle = enableAction === 'Toggle';
            let isEnabled = enableAction === 'Enable';
            let toggleValue = $ax.style.IsWidgetDisabled(elementIds[i]);
            for(let j = 0; j < elementIds.length; j++) {
                $ax('#' + elementIds[j]).enabled(isToggle ? toggleValue : isEnabled);
            }
        }
        _dispatchAction(eventInfo, actions, index + 1);
    };
